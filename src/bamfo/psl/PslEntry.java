...
class PslEntry {
    ...
    /**
     * Creates a cigar string from the psl coordinates. 
     * 
     * @return 
     */
    public String getCigar() {
        ...
        for (int i = 1; i < numblocks; i++) {
            int nowqpos = qStarts.get(i);
            int nowtpos = tStarts.get(i);
            int prevtpos = tStarts.get(i - 1);
            int prevqpos = qStarts.get(i - 1);
            int prevblock = blockSizes.get(i - 1);
        ...
        cigar.append(blockSizes.get(numblocks - 1)).append("M");
        querypos += blockSizes.get(numblocks - 1);

        // check if there is final position clipping
        if (querypos < Qsize) {
            cigar.append(Qsize - querypos).append("S");
        }        
